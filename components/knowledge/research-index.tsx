...
export function ResearchIndex() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      <section className="mx-auto flex max-w-6xl flex-col gap-12 px-6 pb-24 pt-32 md:px-10 lg:pt-44">
        <div className="max-w-3xl">
          <p className="font-mono text-xs uppercase tracking-[0.24em] text-muted-foreground">AKSOS / KNOWLEDGE</p>
          ...
          <p className="mt-8 max-w-2xl text-pretty text-lg leading-8 text-muted-foreground">Source-backed research on intelligence systems, AI strategy, digital infrastructure, and the conditions that shape participation.</p>
        </div>
        <KnowledgeSearch />
...
export function ResearchDetail({ slug }: { slug: string }) {
  ...
  if (!item) return null
  return <main className="min-h-screen bg-background text-foreground"><article className="mx-auto max-w-4xl px-6 pb-24 pt-32 md:px-10 lg:pt-44"><Link href="/research" className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground hover:text-foreground">← Research index</Link><p className="mt-16 font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Research / {item.updated}</p><h1 className="mt-6 text-balance font-serif text-5xl leading-[0.98] tracking-[-0.04em] md:text-7xl">{item.question}</h1><div className="mt-14 border-y border-border py-10"><p className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Direct answer</p><p className="mt-5 max-w-3xl font-serif text-2xl leading-tight md:text-4xl">{item.directAnswer}</p></div><div className="mt-16 grid gap-16 md:grid-cols-[1fr_0.32fr]"><div className="flex flex-col gap-14"><ResearchSection title="Why it matters" text={item.whyItMatters} /><ResearchSection title="The landscape" text={item.landscape} /><ResearchSection title="Evidence" text={item.evidence} /><ResearchSection title="AKSOS analysis" text={item.analysis} /><ResearchSection title="Limitations" text={item.limitations} /></div><aside className="flex flex-col gap-10"><div><p className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Entities</p><ul className="mt-4 flex flex-col gap-3">{item.entities.map((entity) => <li key={entity.id} className="text-sm">{entity.name}</li>)}</ul></div><div><p className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Sources</p><ul className="mt-4 flex flex-col gap-4">{item.sources.map((source) => <li key={source.id}><a href={source.url} target="_blank" rel="noreferrer" className="text-sm leading-6 underline decoration-border underline-offset-4 hover:decoration-foreground">{source.organization}: {source.title}</a><p className="mt-1 text-xs text-muted-foreground">{source.publishedDate}</p></li>)}</ul></div></aside></div><div className="mt-20 border-t border-border pt-10"><p className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Related research</p><div className="mt-6 grid gap-px border border-border bg-border md:grid-cols-2">{item.relatedQuestions.map((related) => { const r = research.find((entry) => entry.id === related); return r ? <Link key={r.id} href={`/research/${r.slug}`} className="bg-background p-5 text-sm hover:bg-muted/40">{r.question}</Link> : null })}</div></div></article></main>
}
